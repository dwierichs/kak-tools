...
def bdi(u, p, q, is_horizontal=True):
    ...
    assert np.allclose([det(k2), det(k2[:q, :q]), det(k2[q:, q:])], 1.)
    ...
    while decomposed_something:
        ...
        for k, (op, start, end, _type) in enumerate(ops[_iter]):
            ...
            for s, e in locs:
                ...
                new_ops.extend([(k1, start+s, start+e, "k1"), (a, start+s, start+e, "a"), (k2, start+s, start+e, "k2")])
            decomposed_something = True
        _iter += 1
        ops[_iter] = new_ops
        if _iter == num_iter:
    ...
    for i in range(1, num_iter+1):
        ...
        for k, (op, start, end, _type) in enumerate(ops[i-1]):
            ...
            for s, e in [(0, width//2), (width//2, width)]:
                ...
                new_ops.extend([(k1, start+s, start+e, "k"), (a, start+s, start+e, "a"), (k2, start+s, start+e, "k")])
        ops[i] = new_ops
    return ops
