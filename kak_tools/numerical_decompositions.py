...
def aii_kak(u, J=J_n, validate=_validate_default):
    ...
    if validate:
        ...
        assert np.allclose(J @ s2.conj() @ J.T, s2, atol=1e-6), f"\n{J @ s2.conj() @ J.T=}\n{s2=}" # s2 is symplectic
...
def aiii_kak(u, p, q, validate=_validate_default):
    ...
    if validate:
        ...
        r = min(p, q)
        s = max(p, q)
        assert np.allclose(k1[:p, p:], 0.) and np.allclose(k1[p:, :p], 0.)
        assert np.allclose(k1 @ k1.conj().T, np.eye(dim))
        assert np.allclose(k2[:p, p:], 0.) and np.allclose(k2[p:, :p], 0.)
        ...
        assert np.allclose(f[r:s, :r], 0.)
        assert np.allclose(f[r:s, s:], 0.)
        assert np.allclose(f[:r, r:s], 0.)
        assert np.allclose(f[s:, r:s], 0.)
...
def bdi_kak(o, p, q, validate=_validate_default):
    ...
    assert np.isclose(d1p * d1q * d2p * d2q, 1.), f"{d1p * d1q * d2p * d2q}"
    s = max(p, q)
    ...
    if validate:
        ...
        assert np.allclose(k1.imag, 0.) and np.allclose(k2.imag, 0.)
        ...
        assert np.allclose([det(k1[:p, :p]), det(k1[p:, p:]), det(k2[:p, :p]), det(k2[p:, p:])], 1.)
...
def diii_kak(o, J=J_n, validate=_validate_default):
    ...
    if validate:
        assert np.allclose(u1.imag, 0.) # u1 is orthogonal
        ...
        assert np.allclose(u2.imag, 0.) # u1 is orthogonal
        assert np.allclose(u2 @ u2.conj().T, np.eye(dim), atol=1e-6) # u2 is unitary
        assert np.allclose(J @ u2.conj() @ J.T, u2, atol=1e-6), f"\n{J @ u2.conj() @ J.T=}\n{u2=}" # u2 is symplectic
