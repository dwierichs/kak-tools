...
def find_horizontal(h, majoranas, maja_dict, verbose=0):
    ...
    for mapping in GM.subgraph_isomorphisms_iter():
        if verbose: print('iso_time: ', time()-t)
        ...
        for i in range(n):
            ...
            #print(maja_inds)
            if len(maja_inds) > 1:
                new_ordering[i + n] = majoranas[set.intersection(*[set(lst) for lst in maja_inds]).pop()]
            ...
            #print(maja_inds)
            if len(maja_inds) > 1:
                new_ordering[i] = majoranas[set.intersection(*[set(lst) for lst in maja_inds]).pop()]
            if verbose: print(new_ordering)
        ...
        for i in range(n):
            ...
            if len(maja_inds) == 1:
                ...
                elif majoranas[maja_inds[0][1]] in new_ordering and not(majoranas[maja_inds[0][0]] in new_ordering):
...
def horizontal_mf_graph(n):
    ...
    for y in range(n):
        ...
        G.add_edges_from([(v1, v2) for i, v1 in enumerate(row_vertices) 
                           for v2 in row_vertices[i+1:]])
    
    # Add vertical edges (entire column)
    for x in range(n):
        col_vertices = [(x, y) for y in range(n)]
        G.add_edges_from([(v1, v2) for i, v1 in enumerate(col_vertices) 
                           for v2 in col_vertices[i+1:]])
...
def verify(m, k1, a, k2, verify_sparse=False):
    ...
    if verify_sparse:
        ...
        assert np.allclose(full_u, k1f @ af @ k2f) or np.allclose(full_u, -k1f @ af @ k2f), (logm(k1), det(k1[:n,:n]), det(k1[n:,n:]))
