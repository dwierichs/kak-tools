...
class TestFDHSCompilation:
    ...
    def test_tfXY_model(self, n):
        ...
        pauli_decomp = map_recursive_decomp_to_reducible(
            recursive_decomp, mapping, signs, time=epsilon, validate=False
        )
        paulirot_decomp = [
            (coeff, qml.pauli.pauli_word_to_string(pw), pw.wires, _type)
            for pw, coeff, _type in pauli_decomp
        ]
        ...
        N = qml.dot([(1 - r) / 2 for r in range(1, n + 1)], Zs) + sum(
            [(r - 1) / 2 for r in range(1, n + 1)]
        )
