...
def make_doubled_orthogonal(n):
    ...
    return np.block(
        [[make_orthogonal(n), np.zeros((n, n))], [np.zeros((n, n)), make_orthogonal(n)]]
    )
...
def make_doubled_symplectic(n):
    ...
    return np.block(
        [
            [make_symplectic(n), np.zeros((2 * n, 2 * n))],
            [np.zeros((2 * n, 2 * n)), make_symplectic(n)],
        ]
    )
...
def assert_special_orthogonal(x):
    ...
    assert np.isclose(np.linalg.det(x), 1.0)
...
def assert_block_unitary(x, p, q):
    ...
    assert np.allclose(x01, 0.0) and np.allclose(x10, 0.0)
    assert_unitary(x00)
    assert_unitary(x11)
    return x00, x11
...
def assert_block_symplectic(x, p, q):
    ...
    for i, j in [(0, 1), (0, 3), (1, 0), (1, 2), (2, 1), (2, 3), (3, 0), (3, 2)]:
        assert np.allclose(x_splits[i][j], 0.0)
...
def assert_schur(x):
    if x.shape[0] % 2:
        ...
        idx = np.where(np.isclose(np.diag(x), 1.0))[0][0]
        zeros = [x[idx, :idx], x[idx, idx + 1 :], x[:idx, idx], x[idx + 1 :, idx]]
        assert all(np.allclose(z, 0.0) for z in zeros)
        sliced_x = np.block(
            [[x[:idx, :idx], x[:idx, idx + 1 :]], [x[idx + 1 :, :idx], x[idx + 1 :, idx + 1 :]]]
        )
    ...
    assert np.allclose(C0**2 + S0[::2] ** 2, 1.0)
    rest = x - np.diag(D) - np.diag(S0, k=1) - np.diag(S1, k=-1)
    assert np.allclose(rest, 0.0)
...
def assert_cossin(x, p, q):
    ...
    assert np.allclose(C0, C1)
    assert np.allclose(S0, -S1)
    assert np.allclose(np.diag(C0) ** 2 + np.diag(S0) ** 2, 1.0)
    eye = x[r:s, r:s]
